...
from sqlalchemy import func, cast, Integer, case, or_
...
# ----------------------------------------------------------------------
# Quick Search – role + location with simple trend deltas
# ----------------------------------------------------------------------
...
def quick_search():
    ...
    # Keep a query object for the template (quick_search.html uses query.role/query.location)
    query = {
        "role": role_raw,
        "location": loc_raw,
    }
    ...
    # Canonical role expression: prefer job_role_group, fallback to job_role
    ...
    summary = {
        ...
        "min_rate": float(min_rate or 0.0),
        "max_rate": float(max_rate or 0.0),
    ...
    # Only keep rows where year/month resolved to integers.
    buckets_q = (
        base_q.with_entities(
    ...
    # If you later want a JSON API, you can add a format=json branch here.
